# =============================================================================
# app.py  —  MARKET TERMINAL  v5
# • Ticker: full-width, large, readable
# • Clock: JavaScript — updates every second, no server round-trip
# • Data:  st.fragment(run_every=300) — refreshes every 5 min, no rate-limit
# =============================================================================
...
from data.fetcher import (
    ...
    get_volatility_data, prefetch_all,
)
...
/* ══════════════════════════════════════════
   TICKER TAPE  — big, full-width, readable
   ══════════════════════════════════════════ */
.tkr-outer {
  width: 100vw;
  position: relative;
...
.tkr-track {
  ...
  white-space: nowrap;
  animation: tkr 240s linear infinite;
}
...
.ti-s { color: #ffffff; font-weight: 700; font-size: 15px; }
.ti-p { color: #444;    font-weight: 400; font-size: 13px; }
...
.mkt-cd { font-size: 13px; font-weight: 600; margin-top: 5px; color: #aaa; }
...
.pt-sym     { font-size: 22px; font-weight: 800; color: #fff; }
...
.sec-n     { color: #c8c8c8; font-size: 11px; font-weight: 500; }
...
.y-n   { color: #c8c8c8; font-size: 12px; }
...
.neg { color: #ff1744 !important; }
.acc { color: #00bcd4 !important; }
...
/* ══════════════════════════════════════════
   BADGE
   ══════════════════════════════════════════ */
.mb    { display: inline-block; font-size: 8px; font-weight: 700;
         letter-spacing: 2px; padding: 2px 7px; border-radius: 1px; text-transform: uppercase; }
...
# =============================================================================
# TICKER  — outside fragment so animation never resets
# =============================================================================
    ...
    for h in header:
        if h["type"] == "date":
            ...
            items.append(
                ...
                f'</span>')
    s = "".join(items)
    return (f'<div class="tkr-outer">'
            f'<div class="tkr-track">{s}{s}</div>'
            f'</div>')

st.markdown(get_ticker_html(), unsafe_allow_html=True)

# =============================================================================
# TOP ROW FRAGMENT — runs every 60 s so the clock stays accurate.
# Indices are served from cache; only a cache miss hits yfinance.
# =============================================================================
...
def top_row():
    ...
    time_str = now_et.strftime("%-I:%M %p").lower()   # e.g. "4:32 pm"
    ...
    st.markdown(
        ...
        f'<div class="clock-sub">NEW YORK · ET</div>'
        f'</div>'
...
# =============================================================================
# DATA FRAGMENT — refreshes MCI, portfolio, sectors, risk, yields every 5 min.
# =============================================================================
@st.fragment(run_every=60)
def dashboard():
    prefetch_all()
    ...
    with col_mci:
        ...
        fbars = "".join(
            f'<div class="fb-row">'
            f'<div class="fb-top"><span>{fn.replace("_"," ").title()}</span>'
    ...
    with col_port:
        ...
        x_pct  = xeqt.get(KEY); b_pct = btc.get(KEY)
        ...
        table = (
            ...
            f'<div class="pt-px acc">{fp(xeqt.get("price"))}</div>'
            f'<div class="pt-ch {cl(x_pct)}">{ar(x_pct)}{fpc(x_pct)}</div>'
            f'<div style="text-align:right;">'
            f'<div class="pt-ret {cl(ret)}">{fpc(ret)}</div>'
            f'<div class="pt-wt">80% WEIGHT</div></div></div>'
            f'<div class="pt-r">'
            f'<div class="pt-sym">BTC</div>'
            f'<div class="pt-px gld">${fp(btc.get("price"),0)}</div>'
            f'<div class="pt-ch {cl(b_pct)}">{ar(b_pct)}{fpc(b_pct)}</div>'
            f'<div style="text-align:right;">'
            f'<div class="pt-wt" style="margin-top:14px;">20% WEIGHT</div>'
            f'</div></div>')

        st.markdown(
            f'<div class="card">'
            f'<div class="card-hdr"><span>Portfolio · 80% XEQT / 20% BTC</span>'
            f'{badge(MODE)}</div>'
            f'{stats}{table}</div>',
            unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.markdown(
            ...
            f'<div>{sec_col(left)}</div>'
            f'<div>{sec_col(right)}</div>'
            f'</div></div>',
            unsafe_allow_html=True)
